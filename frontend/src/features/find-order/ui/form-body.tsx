...
import { Button, DatePicker, Input, Title } from '../../../shared/ui';
import { FC, useEffect } from 'react';
import { IFormOrdersSearchValues } from '../constants';
import { EFormOrdersSearchNames, FORM_NAMES } from '../constants/form-names.ts';
...
export const FormBody: FC<FormBodyProps> = ({ isDisableSubmit }) => {
  const { setValues, values } = useFormikContext<IFormOrdersSearchValues>();

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);

    void setValues((formValues) => {
      for (const key of FORM_NAMES) {
        const valueFromSearchParams = params.get(key);
        if (!valueFromSearchParams) {
          continue;
        }

        if (key === EFormOrdersSearchNames.loadingDate) {
          formValues[key] = dayjs(valueFromSearchParams);
        } else {
          formValues[key] = valueFromSearchParams;
        }
      }
      ...
      <CardBody>
        ...
        <Field name={EFormOrdersSearchNames.loadingDate}>
          {({
            ...
            meta: { error, touched },
          }: FieldProps<Dayjs, IFormOrdersSearchValues>) => {
